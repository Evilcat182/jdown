...
def _get_excluded(path: str, settings: list = config.get_config("organizer_excludes")) -> set:
...
def _find_videos(path: Path, excluded: set = None) -> list[Path]:
    result = []
    for f in path.rglob("*"):
        if not f.is_file() or f.suffix.lower() not in config.get_config("video_extensions"):
            continue
...
def _resolve_content_root(src: Path) -> Path:
    ...
    video_files = [e for e in entries if e.is_file() and e.suffix.lower() in config.get_config("video_extensions")]
...
def _copy_extras(src: Path, out_dir: Path, excluded: set = None):
    ...
    for f in src.rglob("*"):
        if not f.is_file():
            continue
        if f.suffix.lower() in config.get_config("video_extensions"):
            continue
        if excluded and _is_excluded(f, excluded):
            continue
...
def _organize_movie(src: Path, dest: Path, info: dict, videos: list[Path], excluded: set = None, error_path: Path = None) -> bool:
    ...
    if missing:
        ...
        missing = _check_mandatory(merged, config.get_config("movie_settings")["mandatory"])
        if not missing:
    ...
    moved = False
    for video in videos:
        filename = _render_template(config.get_config("movie_settings")["file_template_name"], info, sep=".") + video.suffix.lower()
...
def _organize_series(src: Path, dest: Path, info: dict, videos: list[Path], excluded: set = None, error_path: Path = None) -> bool:
    ...
    for video, merged in merged_per_video:
        ...
        episode_folder_template = config.get_config("series_settings").get("episode_folder_template_name")
        ...
        filename = _render_template(config.get_config("series_settings")["file_template_name"], merged, sep=".") + video.suffix.lower()
    ...
    for f in src.rglob("*"):
        if not f.is_file() or f.suffix.lower() in config.get_config("video_extensions"):
            continue
        if excluded and _is_excluded(f, excluded):
            continue
...
def organize(src: Path, info_override: dict = None):
    ...
    if moved and config.get_event("plex").is_set():
        if media_type == "movie":
            plex_scan_library("movie")
        elif media_type == "episode":
            plex_scan_library("show")

    if moved and config.get_event("delete_source").is_set():
        try:
            shutil.rmtree(src)
